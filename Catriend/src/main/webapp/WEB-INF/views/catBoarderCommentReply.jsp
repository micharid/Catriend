...
<body>
 ...
 %>
					...
					<div class="panel-footer">
						...
						<table class="table">
							...
							<c:forEach items="${CatBoarderCommentLists}" var="row">
								...
								<c:if test="${cbc_index == row.cbc_index}">
									<form action="catBoarderCommentReplyAction"
										name="replyCommentFrm" onsubmit="return checkReplyFrm();"
										method="post">
										<tr>
											<td colspan="3"><input type="hidden" name="nowPage"
												value="${nowPage}" /> <input type="hidden" name="cbc_index"
												value="${cbc_index}" /> <input type="hidden"
												name="cb_index" value="${cb_index}" /> <input type="hidden"
												name="u_id" value="${loginUser.u_id}" /> <textarea
													id="textAreaComment" cols="30" rows="5" name="cbc_content"
													placeholder="댓글을 입력해 주세요"></textarea></td>
											<td align="center"><button type="submit"
													class="btn btn-info">등록</button></td>
										</tr>
									</form>
								</c:if>
							</c:forEach>
			...
			<script>
				document.replyCommentFrm.cbc_content.focus();
			</script>
		</div>
	</div>
